...
| 6 | ✅ | **测试框架迁移（node:test → vitest）** | 53 个测试文件从 `node:test` 迁移到 `vitest`，利用 vitest 单进程 + 原生 TypeScript 支持，`npm test` 耗时从 **30.6s → 1.9s**（16x 提升）。迁移内容：`vitest.config.ts` 配置、`#src/*` 路径别名、`mock.method()` → `vi.spyOn().mockImplementation()`、`mock.fn()` → `vi.fn()`、`console.log` mock 适配等 |
...
- **FIXED：`--skip-permission` 启动参数不生效** — CLI 参数解析后通过 `setDefaultContext({skipPermission})` 存入 `defaultCtx`，但 agent 权限检查使用 `isSkipPermissionScope()` 读取独立的 `_skipPermissionScope`（初始化为 `false`），`setSkipPermissionScope` 在生产代码中从未被调用，导致两存储位置断链。修复：在 `initializePlugins` 中增加 `registry.setSkipPermissionScope(skipPermission)`。(2026-07-28)
