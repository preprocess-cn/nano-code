...
const BUILTIN_LOADERS: Record<string, (settings?: Record<string, any>) => Promise<NanoPlugin>> = {
  ...
  skills: async (s) => (await import('#src/plugins/skills/index.js')).createSkillsPlugin(s?.llmClient, s?.displayMgr, s?.agentManager, { disabled: s?.disabled, disableSkillTool: s?.disableSkillTool, skillsDirs: s?.skillsDirs }),
