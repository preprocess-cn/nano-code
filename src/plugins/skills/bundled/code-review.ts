...
/**
 * Code Review 技能 — 深度代码审查。
 *
 * 对齐 Claude Code 的 `/code-review` command：
 * 7 angles × 6 candidates → 1-vote verify → ≤10 findings JSON
 *
 * simplify = code-review + --fix（参见 simplify.ts）
 */
export function createCodeReviewSkill(title?: string): BundledSkillDef {
  return {
    name: 'code-review',
    description: 'Deep code review with multi-angle analysis',
    argumentHint: '[PR number | additional context]',
    whenToUse: 'use when the user asks to deeply review, audit, or analyze code changes',
...
Run \`run_bash_command\` to execute \`git diff @{upstream}...HEAD\` (or \`git diff main...HEAD\` / \`git diff HEAD~1\` if there's no upstream) to get the unified diff under review. If there are uncommitted changes, or the range diff is empty, also run \`git diff HEAD\` and include the working-tree changes in scope — the review often runs before the commit. ${!hasPrArg && args ? `The user also passed this context: ${args}` : ''}${hasPrArg ? `Review PR #${args.trim()} — use \`gh pr view ${args.trim()}\` and \`gh pr diff ${args.trim()}\` to get the details.` : ''}
