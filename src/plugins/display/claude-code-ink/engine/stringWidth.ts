...
/**
 * Fallback JavaScript implementation of stringWidth when the JS fallback is not available.
 *
...
// consonants). the JS fallback=2 matches terminal cell allocation, which is what
// we need for cursor positioning — the JS fallback's grapheme-cluster width of 1
// would desync Ink's layout from the terminal.
//
// the JS fallback is resolved once at module scope rather than checked on every
