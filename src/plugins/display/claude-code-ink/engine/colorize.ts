...
import type { Color, TextStyles } from '#src/plugins/display/claude-code-ink/engine/styles.js'
...
export function applyTextStyles(text: string, styles: TextStyles): string {
  ...
  if (styles.dim) {
    // Hardcode ANSI dim codes instead of chalk.dim, which produces nothing
    // at chalk.level < 1. This gives reliable dim across all terminal
    // environments regardless of chalk's level detection.
